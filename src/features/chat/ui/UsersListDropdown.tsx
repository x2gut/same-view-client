...
const UsersListDropdown = ({
  ...
  return (
    <div className="absolute right-0 z-10 w-64 py-1 mt-2 overflow-hidden border bg-[var(--card)] border-[var(--accent)] shadow-lg rounded-md">
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--accent)]">
        <h3 className="text-sm font-medium text-[var(--text)]">Chat Users</h3>
        <span className="text-xs text-[var(--text)]">{users.length} users</span>
      </div>

      {showSearch && (
        <div className="px-3 py-2 border-b border-[var(--accent)]">
      ...
      <div className="overflow-y-auto" style={{ maxHeight }}>
        {filteredUsers.length > 0 ? (
          ...
          <div className="px-3 py-4 text-sm text-center text-[var(--text)]">
            No users found
          </div>
        )}
      </div>
